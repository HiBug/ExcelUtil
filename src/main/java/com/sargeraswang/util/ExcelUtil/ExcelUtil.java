...
public class ExcelUtil {
	...
	public static <T> void exportExcel(Map<String, Collection<Map>> data, OutputStream out, String pattern) {
		// 声明一个工作薄
		XSSFWorkbook workbook = new XSSFWorkbook();

		for (Map.Entry<String, Collection<Map>> entry : data.entrySet()) {
			// 生成一个表格
			XSSFSheet   sheet = workbook.createSheet(entry.getKey());
			Set<String> title = null;
			if (entry.getValue().iterator().hasNext()) {
				title = entry.getValue().iterator().next().keySet();
			} else {
				continue;
			}
	 ...
	 */
	public static <T> Collection<T> importExcel(String sheetName, Class<T> clazz, InputStream inputStream,
